...
browser.runtime.onInstalled.addListener(function (details) {
  ...
  browser.tabs.create({ url: 'permissions/permissions.html' });
  (async () => {
    try {
      ...
      await browser.storage.local.set({ 'aiProvidersMap': providersMap });
    } catch (error) {
...
browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'complete' && tab.url) {
    try {
        ...
        if (hasPermission && urlRegex.test(tab.url)) {
          ...
          try {
            ...
            console.log(`Successfully injected scripts into tab ${tabId}`);
             ...
             if (injectionError.message.includes('Cannot access a browser:// URL') || injectionError.message.includes('No matching window')) {
      ...
      // Avoid logging errors for URLs like 'browser://extensions/'
      if (tab.url && !tab.url.startsWith('browser://')) {
